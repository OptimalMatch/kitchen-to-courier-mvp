...
export class Node {
  ...
  async post(path, body, retried = false) {
    ...
    if (!res.ok || data.error) {
      ...
      if (!retried && body && body.collection && /needs every member local/.test(String(data.error || text))) { await ensureLocal(this, `${body.collection}.`); return this.post(path, body, true); }
