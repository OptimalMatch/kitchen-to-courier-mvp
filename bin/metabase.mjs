...
const QUESTIONS = [
  ...
  { db: 1, name: "Settlement vs sales (step 7)", sql: "SELECT restaurant_id, count(*) AS disputed_lines, sum(CASE WHEN sales_net_cents IS NULL THEN 1 ELSE 0 END) AS no_till_record, sum(-mismatch_cents) AS till_over_cents FROM settlement_mismatches GROUP BY restaurant_id ORDER BY restaurant_id" },
